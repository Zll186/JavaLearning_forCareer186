...
```java
class Solution {
    private final int[][] DIRS = {{0,1},{1,0},{0,-1},{-1,0}}; //定义移动方向(右下左上)
    public List<Integer> spiralOrder(int[][] matrix) {        
        int m= matrix.length;                                 //当前还有多少行m
        int n = matrix[0].length;                             //当前还有多少列n
        int size =m*n;                                        //初始元素总个数
        List<Integer> ans =new ArrayList<>(size);             //初始ans：List<>
        int i=0;                                              //当前坐标i0，j-1
        int j=-1;                                             //因为第一轮方向是向右，进入循环后会先移动一步：
        for(int dis =0;ans.size()<size;dis=(dis+1)%4){        //外层for：dis控方向，没收集满size个元素，就继续走
            for(int k=0;k<n;k++){                                 //内层for：当前方向连续走n步
                i+=DIRS[dis][0];                                      //更新行坐标i  
                j+=DIRS[dis][1];                                      //更新列坐标j
                ans.add(matrix[i][j]);}                               //每走一步就把当前位置元素加入ans。
            int tmp =n;                                            //横竖切换 + 缩掉一层
            n=m-1;
            m = tmp; 
        }
